  ...
  <div class="row">
    <div class="col-12 col-lg-8">
                  ...
                  <td class="sorting_1">
                    <div class="d-flex justify-content-start align-items-center product-name">
                      ...
                      <div class="d-flex flex-column">
                        ...
                        <small
                          class="text-truncate d-none d-sm-block">{{ Str::limit($order->product->description, 100) }}</small>
                      </div>
                    </div>
                  </td>
                </tr>
                  ...
                  <div class="d-flex justify-content-start align-items-center my-2">
                    <div class="d-flex flex-column align-items-start justify-content-end">
                      ...
                      <span class="text-truncate">
                        Subtotal Harga &emsp; : Rp {{ number_format($order->total_price, 0, ',', '.') }}
                      </span>
                      <span class="text-truncate">
          ...
          <ul class="timeline pb-0 mb-0">
            ...
            @if ($order->payment_method)
              <li class="timeline-item timeline-item-transparent border-primary">
                ...
                <div class="timeline-event">
                  <div class="timeline-header mb-1">
                    <h6 class="mb-0">Pesanan anda berhasil dibayar menggunakan <span
                  ...
                  <p class="mt-1 mb-3">Pesanan anda sedang diproses</p>
                </div>
              </li>
            @endif
                ...
                <div class="timeline-event">
                  <div class="timeline-header mb-1">
                    <h6 class="mb-0">Status pesanan anda <span
                        class="text-uppercase badge bg-label-success">{{ $order->status }}</span></h6>
                    <small class="text-muted">{{ date('d M Y, H:i:s', strtotime($order->updated_at)) }}</small>
                  </div>
      ...
      <div class="d-flex justify-content-center align-items-center gap-3 mb-3">
        @if ($order->status === 'paid')
          <x-submit-button :label="'Preview'" :type="'submit'" :class="'w-100'" :variant="'outline-primary'" :icon="'note-search-outline'" />
          <x-submit-button :label="'Cetak'" :type="'submit'" :class="'w-100'" :variant="'primary'" :icon="'file-download-outline'" />
        ...
        <div class="card-body">
          ...
          @if ($order->status == 'paid')
            ...
            <p class="mb-1">Tanggal Pembayaran :
              <span
                class="text-uppercase badge bg-label-info rounded">{{ date('d M Y, H:i:s', strtotime($order->updated_at)) }}
          ...
          @elseif($order->status == 'unpaid')
            ...
            <p class="mb-1">Bayar Sebelum :
              <span
                class="text-uppercase badge bg-label-info rounded">{{ date('d M Y, H:i:s', strtotime($order->expiry_time)) }}
                {{-- {{ $order->expiry_time->format('H:i') > '12:00' ? 'PM' : 'AM' }} --}}
              </span>
            </p>
          @endif
          @if ($order->status == 'paid')
