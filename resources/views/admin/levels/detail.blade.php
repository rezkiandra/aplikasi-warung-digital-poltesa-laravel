    ...
    <div class="card-body">
      ...
      <div class="d-flex justify-content-between flex-wrap my-2 py-3">
        <div class="d-flex align-items-center me-4 mt-3 gap-3">
          ...
          <div>
            <h4 class="mb-0">{{ $level->level_name }}</h4>
        ...
        <div class="d-flex align-items-center me-4 mt-3 gap-3">
          ...
          <div>
            <h4 class="mb-0">{{ $level_user_count }}</h4>
      ...
      <div class="info-container">
        <ul class="list-unstyled mb-4">
          ...
          <li class="mb-3">
            ...
            <span
              class="badge bg-label-success rounded-pill">{{ date('d F Y, H:i:s', strtotime($level->created_at)) }}</span>
          </li>
          <li class="mb-3">
            <span class="h6">Updated At:</span>
            <span
              class="badge bg-label-info rounded-pill">{{ date('d F Y, H:i:s', strtotime($level->updated_at)) }}</span>
          </li>
        </ul>
        <div class="d-flex justify-content-center align-items-center gap-3 mt-5">
          <x-basic-button :label="'Edit'" :icon="'pencil-outline'" :href="route('admin.edit.level', $level->slug)" :variant="'primary'" />
          <form action="{{ route('admin.destroy.level', $level->slug) }}" method="POST">
            @csrf
            @method('DELETE')
